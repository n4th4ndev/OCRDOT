...
# Try to import LocalNanonetsProcessor, but don't fail if dependencies missing
try:
    from .processors.local_nanonets_processor import LocalNanonetsProcessor
    LOCAL_NANONETS_AVAILABLE = True
except ImportError as e:
    logger.warning(f"LocalNanonetsProcessor not available: {e}")
    LOCAL_NANONETS_AVAILABLE = False
    LocalNanonetsProcessor = None
...
class DocumentExtractor:
    ...
    def __init__(
        ...
        Note:
            ...
            - Without API key, limited free calls per day
            - For increased limits, use an API key from https://app.nanonets.com/#/keys
        """
        ...
        if self.cloud_mode and not self.api_key:
            ...
            # Skip automatic authentication - let user explicitly authenticate if needed
            # This prevents automatic Google auth popup
            if not self.api_key:
                logger.info("No API key provided - using limited free cloud processing")
        
        # Initialize processors
        self.processors = []

        # Use local Nanonets model as primary processor (optimized for VPS with GPU)
        if LOCAL_NANONETS_AVAILABLE:
            local_processor = LocalNanonetsProcessor()
            self.processors.append(local_processor)
            logger.info("Local Nanonets-OCR2-3B model enabled (optimized for VPS with GPU)")
        else:
            logger.info("Local Nanonets processor not available (missing dependencies)")
        
        # Keep cloud processor as fallback if API key is available
        if self.cloud_mode and self.api_key:
            ...
            logger.info("Cloud processing enabled as fallback with authenticated access")
        elif self.cloud_mode:
            logger.info("Cloud processing available as fallback (limited free calls)")
        
        # Setup local processors for non-OCR tasks
    ...
    def _get_processor(self, file_path: str):
        ...
        # First, try local Nanonets processor for supported formats
        for processor in self.processors:
            if hasattr(processor, '__class__') and 'LocalNanonetsProcessor' in str(processor.__class__):
                if hasattr(processor, 'is_supported') and processor.is_supported(file_path):
                    logger.info(f"Using Local Nanonets-OCR2-3B processor for {file_path}")
                    return processor
        ...
        for processor in self.processors:
            if hasattr(processor, 'can_process') and processor.can_process(file_path):
        ...
        for processor in self.processors:
            if hasattr(processor, 'can_process'):
                ...
                if isinstance(processor, PDFProcessor):
                    ...
                    # Cloud processor supports many formats
                    formats.extend(['.pdf', '.docx', '.doc', '.txt', '.xlsx', '.xls', '.csv', '.pptx', '.ppt', '.html', '.jpg', '.jpeg', '.png', '.bmp', '.tiff', '.webp', '.gif'])
                elif hasattr(processor, '__class__') and 'LocalNanonetsProcessor' in str(processor.__class__):
                    # Local Nanonets processor supports image and PDF formats
                    formats.extend(['.jpg', '.jpeg', '.png', '.bmp', '.tiff', '.webp', '.pdf'])
