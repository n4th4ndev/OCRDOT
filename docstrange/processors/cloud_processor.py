                ...
                if response.status_code == 429:
                    if not self.cloud_processor.api_key:
                        error_msg = (
                            ...
                            "Use an API key from https://app.nanonets.com/#/keys for increased limits.\n"
                            "Examples:\n"
                            "  - CLI: docstrange document.pdf --api-key YOUR_API_KEY\n"
                            "  - Python: DocumentExtractor(api_key='YOUR_API_KEY')\n"
                            "  - Env var: export NANONETS_API_KEY=YOUR_API_KEY"
                        )
...
class CloudProcessor(BaseProcessor):
    ...
    def process(self, file_path: str) -> CloudConversionResult:
        ...
        if self.api_key:
            logger.info(f"Created cloud extractor for {file_path} with API key")
        else:
            logger.info(f"Created cloud extractor for {file_path} without API key - free tier with rate limits")
